...
// refs so they can be cleaned up safely.
export default React.memo(function Header() {
  ...
  const splitRef = useRef(null); // instance of SplitText
  ...
  // Helper to move the indicator under a nav item
  ...
  // Dynamically load GSAP and SplitText once and prepare utilities
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      try {
        ...
        let SplitTextModule = null;
        try {
          SplitTextModule = await import('gsap/SplitText');
        } catch (_) {
          // fallback to all (depends on gsap package)
          SplitTextModule = await import('gsap/all');
        }

        if (!mounted) return;

        gsapRef.current = gsapModule;
        splitCtorRef.current = SplitTextModule?.default || SplitTextModule?.SplitText || null;

        if (gsapModule && typeof gsapModule.matchMedia === 'function') {
          mmRef.current = gsapModule.matchMedia();
        }

        // quickTo utilities used by the indicator
        if (indicatorRef.current && gsapModule) {
          indicatorX.current = gsapModule.quickTo(indicatorRef.current, 'x', {
            duration: 0.3,
            ease: 'power3.out',
          });
          indicatorW.current = gsapModule.quickTo(indicatorRef.current, 'width', {
            duration: 0.3,
            ease: 'power3.out',
          });
          // ensure indicator is positioned initially
          moveIndicator(active);
        }
      } catch (e) {
        // ignore dynamic import failures (still functional without animations)
    ...
    return () => {
      mounted = false;
      try {
        if (mmRef.current && typeof mmRef.current.revert === 'function') mmRef.current.revert();
      } catch (e) {}
    };
  }, [moveIndicator, active]);
  ...
  // Handlers use the GSAP instance if available. Using useCallback prevents
  // unnecessary re-creations and helps React.memo effectiveness.
  const handleLanguageClick = useCallback(
    (e) => {
      ...
      if (isBoxOpen) {
        ...
        if (g) g.to(target, { height: 40, width: 90, duration: 0.4, ease: 'power3.out' });
        setSubMenu(false);
      } else {
      ...
      // split animation (use constructor if available)
  ...
  // Initialize the indicator utilities if gsap is available (safe no-op otherwise)
  useEffect(() => {
    const g = gsapRef.current;
